package cn.monitor.core.query.parse;
...
import cn.monitor.core.query.data.Condition;
import cn.monitor.core.query.data.Condition.Filter;
import cn.monitor.core.query.data.Condition.Operator;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		cn.monitor.core.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (cn.monitor.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.monitor.core.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == cn.monitor.core.query.data.Sort.Direction.ASC) {
