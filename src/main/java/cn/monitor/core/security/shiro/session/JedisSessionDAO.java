package cn.monitor.core.security.shiro.session;

import cn.monitor.core.utils.*;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	public void update(Session session) throws UnknownSessionException {
		...
		if (request != null) {
			...
			// 如果是视图文件，则不更新SESSION
			if (StringUtils.startsWith(uri, WebPropertiesUtil.getConfig("web.view.prefix"))
					&& StringUtils.endsWith(uri, WebPropertiesUtil.getConfig("web.view.suffix"))) {
				return;
			}
			// 手动控制不更新SESSION
